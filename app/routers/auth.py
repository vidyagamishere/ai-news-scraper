...
async def send_otp(
    ...
    try:
        ...
        if DEBUG:
            logger.debug(f"🔍 OTP request details - email: {request.email}, auth_mode: {request.auth_mode}")
        
        # Generate a secure 6-digit OTP
        otp = generate_otp()
        
        if DEBUG:
            logger.debug(f"🔍 Generated OTP for {request.email}: {otp}")
        ...
        if DEBUG:
            logger.debug(f"🔍 Stored OTP in memory for {request.email}")
...
async def verify_otp(
    ...
    try:
        ...
        if DEBUG:
            logger.debug(f"🔍 OTP verification request - email: {request.email}, code: {request.otp}")
            logger.debug(f"🔍 User data: {request.userData}")
        ...
        if request.email in otp_storage:
            if DEBUG:
                logger.debug(f"🔍 Removing stored OTP for {request.email}")
