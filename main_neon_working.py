#!/usr/bin/env python3
"""
AI News Scraper API with Neon Database Support - Working Version
"""
import os
import logging
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("Starting AI News Scraper API with Neon Database")
    
    try:
        # Try to initialize the database service
        database_url = os.getenv("POSTGRES_URL")
        if database_url:
            # Try to import and initialize Postgres support
            try:
                ...
                logger.info(f"✅ Neon database connected - {len(topics)} topics available")
                db_available = True
...
async def root():
    ...
    return {
        ...
        "version": "2.0.0",
        "database": "Neon Postgres" if db_available else "Not Connected",
        "status": "running",
...
async def get_digest():
    ...
    return {
        "summary": {
            "keyPoints": [
                "• Neon PostgreSQL database migration completed",
